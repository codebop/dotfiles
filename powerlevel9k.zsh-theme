# THIS GOES IN ~/.oh-my-zsh/custom/themes/powerlevel9k
...
# AWS Profile
prompt_aws() {
  local aws_profile_zsh="${AWS_PROFILE_ZSH:-$AWS_DEFAULT_PROFILE}"

  if [[ -n "$aws_profile_zsh" ]]; then
    "$1_prompt_segment" "$0" "$2" yello black "$aws_profile_zsh" 'AWS_ICON'
  fi
}
...
prompt_pyenv() {
  if [[ -n "$PYENV_VERSION" ]]; then
    "$1_prompt_segment" "$0" "$2" "green" "$DEFAULT_COLOR" "$PYENV_VERSION" 'PYTHON_ICON'
  fi
}
